...
- 列出六组信号对应的 24 个数字针脚，并提供通电前检查、串口验证和常见接错排查方法。
- README 增加接线说明入口。

对应文件：`开发板接线说明.md`、`README.md`、`PROGRAM_HANDOFF.md`。

## v0.8.1 — 学生及家长使用说明

- 新增独立的 `使用说明.md`，以初次使用顺序说明开发板连接、游戏操作、全部配置项、配置结果、格子信号与针脚关系、胜负规则、配置记忆和常见问题。
- README增加使用说明入口，并修正动态布局下“所有格子”及停止状态不处理命中的表述。

对应文件：`使用说明.md`、`README.md`、`PROGRAM_HANDOFF.md`。

## v0.8.0 — 自适应布局与格子信号配置

- 设置面板新增横2纵3、横3纵2、横2纵2三种界面布局；选择后立即重建正方形格子、目标状态、动画状态和目标到期计时器。
- 新增自适应“格子信号设置”，按当前行列列出每个格子，可选择六位串口信号或“未配置”。六位选项分别注明对应的4个Arduino针脚。
- 选择已被占用的信号时，原占用格自动变为“未配置”；点击“保存信号配置”时校验所有格子均已配置且不存在重复，通过后才影响串口命中路由并写入本地配置。
- 默认映射沿用原程序的反向顺序；横2纵2只绑定第6～3位，未绑定的第1、2位输入被读取但不会命中格子。
- 本地配置只保存当前最新布局和已生效映射；旧配置缺少新字段时自动补入横2纵3及原有映射。
- 自动化测试扩展为39项，覆盖动态布局、针脚注释、冲突清空、保存校验、映射恢复、非法配置回退、实际串口路由、未绑定信号和游戏中设置锁定。

对应代码：`mole_game.py` 的 `GRID_LAYOUTS`、`PIN_GROUPS`、`SquareGrid.set_layout()`、信号映射设置方法与 `register_game_frame()`；`test_game_logic.py`。

## v0.7.2 — 限时游戏与命中分值提示

- 左上角游戏名称上方新增倒计时，默认单局 30 秒；倒计时归零会按既有“游戏结束”流程立即停止输入、清场并显示结束文字。
- 设置面板新增“限时时长”下拉框：20～60 秒、每 10 秒一档；所选值会在开局锁定，并保存至固定本地配置文件后于下次启动恢复。
- 非直接失败的有效命中会在命中格中心显示实际分值变化（正分绿色、负分红色），提示与现有 0.6 秒缩放动画同步结束；`-999` 直接失败不显示分值提示。
- 自动化测试扩展为 31 项，新增倒计时选项/持久化/超时清场与命中分值提示覆盖。

对应代码：`mole_game.py` 的 `GAME_DURATION_OPTIONS_SECONDS`、`countdown_timer`、`game_duration_box`、`SquareGrid.show_score_change()` 与 `MoleGameWindow.hit_target()`；`test_game_logic.py`。
